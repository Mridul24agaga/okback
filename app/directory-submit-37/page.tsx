...
import { saveSelections } from '../actions/saveSelectionss'
...
export default function DirectoryListing() {
  ...
  const handleCheckboxChange = (id: number) => {
    setSelectedDirectories(prev => {
      if (prev.includes(id)) {
        ...
        alert("You can't select more than 50 directories.")
        return prev
      }
    })
  }
  ...
  const selectRandomDirectories = () => {
    ...
    const randomSelected = shuffled.slice(0, 50).map(dir => dir.id)
    setSelectedDirectories(randomSelected)
  }

  const selectAllDirectories = () => {
    const allSelected = directories.slice(0, 50).map(dir => dir.id)
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="md:w-1/3">
            <div className="bg-white rounded-lg shadow-md p-6 sticky top-8">
              <h2 className="text-xl font-bold mb-4">Selected Directories ({selectedDirectories.length}/50):</h2>
